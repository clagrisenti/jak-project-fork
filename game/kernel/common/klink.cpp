  ...
  if (is_opengoal_object(object_file.c())) {
    ...
    if (link_debug_printfs) {
      ...
      for (int i = 0; i < N_SEG; i++) {
        lg::info(" seg {} link 0x{0:#x}, 0x{1:#x} data 0x{2:#x}, 0x{3:#x}", i,
                 ofh->link_infos[i].offset, ofh->link_infos[i].size, ofh->code_infos[i].offset,
                 ofh->code_infos[i].size);
      }
    }
    ...
    if (link_debug_printfs) {
      lg::info("Linking GOAL style object {}", name);
    }
  ...
  do {
    ...
    if (objValue == 0xffffffff) {
      *(objPtr.cast<u32>()) = linkObj.offset;
      ...
      lg::info("val is 0x{0x} ptr {}", objValue, static_cast<const void*>(relocPtr - 1));
      ASSERT(false);
    }
  } while (*relocPtr);
