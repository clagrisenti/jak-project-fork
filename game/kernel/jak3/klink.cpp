...
void link_control::jak3_begin(Ptr<uint8_t> object_file,
  ...
  if (is_opengoal_object(object_file.c())) {
    ...
    if (link_debug_printfs) {
      ...
      lg::info("link {}", m_object_name);
      lg::info("link_control::begin {}{}{}{}", goal_name[0], goal_name[1], goal_name[2],
               goal_name[3]);
    }
    ...
    if (link_debug_printfs) {
      ...
      lg::info(" GOAL ver {}.{} obj {} len {}", ofh->goal_version_major, ofh->goal_version_minor,
               ofh->object_file_version, ofh->link_block_length);
      lg::info(" segment count {}", ofh->segment_count);
      for (int i = 0; i < N_SEG; i++) {
        lg::info(" seg {} link 0x{0x}, 0x{0x} data 0x{0x}, 0x{0x}", i, ofh->link_infos[i].offset,
...
uint32_t link_control::jak3_work_v5() {
  if (m_state == 0) {
    ...
    for (int i = 0; i < m_n_segments; i++) {
      lg::info(" {}: reloc {}, data {}, size {}, magic {}", i, m_link_segments_table[i].relocs,
  ...
  if (m_state == LINK_V2_STATE_INIT_COPY) {  // initialization and copying to heap
    ...
    if (m_heap_gap <
      ...
      if (m_segment_process == 0) {
        ...
        if (link_debug_printfs) {
          lg::info("[work_v2] moving from 0x{0x} to 0x{0x}", m_original_object_location.offset,
    ...
    if (*m_reloc_ptr == 0) {
      ...
      while (true) {
        ...
        if ((relocation & 0x80) == 0) {
          ...
          name = m_reloc_ptr.cast<char>().c();
          if (link_debug_printfs) {
            lg::info("[work_v2] symlink -type: {}", name);
          }
