...
void Profiler::draw_node(ProfilerNode& node, bool expand, int depth, float start_time) {
  if constexpr (profiler::use_profiler) {
    ...
    auto str =
        fmt::format("{:20s} {:.2f}ms {:6d} tri {:4d} draw", node.m_name,
                    node.m_stats.duration * 1000, node.m_stats.triangles, node.m_stats.draw_calls);
...
void ProfilerNode::to_string_helper(std::string& str, int depth) const {
  if constexpr (profiler::use_profiler) {
    str += fmt::format("{}{:.2f} ms {:30s}\n", std::string(depth, ' '), m_stats.duration * 1000,
                       m_name);
    for (const auto& child : m_children) {
