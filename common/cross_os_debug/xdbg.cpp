...
void allow_debugging() {
  ...
  if (prctl(PR_SET_PTRACER, PR_SET_PTRACER_ANY) < 0) {
    lg::info("[Debugger] Failed to PR_SET_PTRACER %s\n", strerror(errno));
  }
}
...
bool attach_and_break(const ThreadID& tid) {
  ...
  if (rv == -1) {
    lg::info("[Debugger] Failed to attach %s\n", strerror(errno));
    return false;
  } else {
    ...
    if (ptrace(PTRACE_INTERRUPT, tid.id, nullptr, nullptr) < 0) {
      lg::info("[Debugger] Failed to PTRACE_INTERRUPT %s\n", strerror(errno));
      return false;
    }
...
bool check_stopped(const ThreadID& tid, SignalInfo* out) {
  ...
  if (rv < 0) {
    ...
    lg::info("[Debugger] Failed to waitpid: %s.\n", strerror(errno));
...
bool open_memory(const ThreadID& tid, MemoryHandle* out) {
  ...
  if (fd < -1) {
    lg::info("[Debugger] Failed to open memory: %s.\n", strerror(errno));
    return false;
  }
  out->fd = fd;
  return true;
...
bool close_memory(const ThreadID& tid, MemoryHandle* handle) {
  ...
  if (close(handle->fd) < 0) {
    lg::info("[Debugger] Failed to close memory: %s.\n", strerror(errno));
    return false;
  }
  return true;
}
...
bool read_goal_memory(u8* dest_buffer,
  ...
  if (pread(mem.fd, dest_buffer, size, context.base + goal_addr) != size) {
    lg::info("[Debugger] Failed to read memory: %s.\n", strerror(errno));
    return false;
  }
  return true;
}
...
bool write_goal_memory(const u8* src_buffer,
  ...
  if (pwrite(mem.fd, src_buffer, size, context.base + goal_addr) != size) {
    lg::info("[Debugger] Failed to write memory: %s.\n", strerror(errno));
    return false;
  }
  return true;
}
...
bool detach_and_resume(const ThreadID& tid) {
  if (ptrace(PTRACE_DETACH, tid.id, nullptr, nullptr) < 0) {
    lg::info("[Debugger] Failed to detach: %s\n", strerror(errno));
    return false;
  }
  return true;
}
...
bool get_regs_now(const ThreadID& tid, Regs* out) {
  ...
  if (ptrace(PTRACE_GETREGS, tid.id, nullptr, &regs) < 0) {
    lg::info("[Debugger] Failed to PTRACE_GETREGS %s\n", strerror(errno));
    return false;
  }
...
bool set_regs_now(const ThreadID& tid, const Regs& out) {
  ...
  if (ptrace(PTRACE_SETREGS, tid.id, nullptr, &regs) < 0) {
    lg::info("[Debugger] Failed to PTRACE_SETREGS %s\n", strerror(errno));
    return false;
  }
  // todo, set fprs.
  return true;
...
bool cont_now(const ThreadID& tid) {
  if (ptrace(PTRACE_CONT, tid.id, nullptr, nullptr) < 0) {
    lg::info("[Debugger] Failed to PTRACE_CONT %s\n", strerror(errno));
    return false;
  }
  return true;
}
...
void win_print_last_error(const std::string& msg) {
  ...
  lg::info("[Debugger] %s Win Err: %s", msg.c_str(), lpMsgBuf);
}
  ...
  if (WaitForDebugEvent(&debugEvent, INFINITE)) {
    ...
    switch (debugEvent.dwDebugEventCode) {
      ...
      case RIP_EVENT:                 // 9
        ...
        break;
      default:
        lg::info("[Debugger] unhandled debug event %lu\n", debugEvent.dwDebugEventCode);
        out->kind = SignalInfo::UNKNOWN;
        break;
    }
