...
FieldPrint get_field_print(const std::string& str) {
  ...
  if (c1 != 'T' && c1 == 'd') {
    lg::info("HACK: skipping {}", str.data());
...
int identify_float_field(int idx,
  ...
  if (!is_op_2(float_move.get(), SimpleExpression::Kind::FPR_TO_GPR, make_gpr(Reg::A2),
               make_fpr(0))) {
    lg::warn("bad float move: {}", float_move->to_string(function.ir2.env).c_str());
    ASSERT(false);
  }
...
bool get_ptr_offset(AtomicOp* ir, Register dst, Register base, int* result) {
  ...
  if (!as_set) {
    lg::warn("not a set, actual type: {}", typeid(*ir).name());
    return false;
  }
...
int identify_array_field(int idx,
  ...
  if (!ptr) {
    lg::warn("bad get ptr offset {}", get_op->to_string(function.ir2.env).c_str());
    ASSERT(false);
  }
  if (result->is_basic) {
    offset += BASIC_OFFSET;
...
int detect(int idx, Function& function, LinkedObjectFile& file, TypeInspectorResult* result) {
  ...
  if ((load_a2_gp || load_v1_gp) &&
    ...
    lg::info("couldn't do {}, {}, adding unknown field", sstr->c_str(),
